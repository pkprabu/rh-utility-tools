...
// --- Set Placeholders ---
sendDateInput.placeholder = "Select Send Date...";
eventStartInput.placeholder = "Select Event Start...";
eventEndInput.placeholder = "Select Event End...";

// --- Initialize Flatpickr ---
flatpickr(inputFields, {
  enableTime: true,
  dateFormat: "Y-m-d\\TH:i", // Logic value
  
  // Display Format: "Wednesday, December 10, 2025 @ 12:00 PM"
  altInput: true,
  // Note: \\@ escapes the @ symbol so it displays as text, not a timestamp
...
eventStartInput.addEventListener("change", () => {
  ...
  if (startVal) {
    // Sync value to End Date
    eventEndInput.value = startVal;
    
    // Sync Flatpickr UI for End Date
    if(eventEndInput._flatpickr) {
      eventEndInput._flatpickr.setDate(startVal);
    }
    updateButtonStates();
  }
});
