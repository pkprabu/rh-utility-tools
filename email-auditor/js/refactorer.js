...
function updateScCid(doc, newScCidValue) {
 ...
 const ignoreList = config.auditor.ignoreList;
 ...
 links.forEach(link => {
     ...
     if (ignoreList.some(prefix => link.href.startsWith(prefix))) {
         return;
     }
     try {
         const url = new URL(link.href);
         const existingValue = url.searchParams.get(parameterName);
         if (!existingValue) {
             url.searchParams.set(parameterName, newScCidValue);
             link.setAttribute('href', url.href);
         }
     } catch (e) {
         console.warn(`Could not parse and update URL: ${href}`, e);
     }
 });
}
...
function fixRedHatSpacing(doc) {
 ...
 nodesToProcess.forEach(textNode => {
     textNode.nodeValue = textNode.nodeValue.replace(incorrectSpacingRegex, 'Red\u00A0Hat');
 });
}
...
export function applyFixes(doc, auditResults, newScCidValue) {
 ...
 auditResults.image_failures.forEach(item => {
     ...
     if (item.failures.includes("Missing/Empty alt text")) {
         img.setAttribute('alt', '');
     }
     if (renderedWidth > 0 && !img.getAttribute('width')) {
         img.setAttribute('width', renderedWidth);
     }
     if (renderedHeight > 0 && !img.getAttribute('height')) {
     ...
     if (renderedWidth > imageSizeThreshold) {
         img.style.width = '100%';
