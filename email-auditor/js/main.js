...
dom.scCidInput.addEventListener('input', debouncedProcessEmail);
dom.htmlInput.addEventListener('input', debouncedProcessEmail);
...
dom.refactorButton.addEventListener('click', () => {
 ...
 setTimeout(() => {
     try {
         const newScCidValue = dom.scCidInput.value.trim();
         const refactoredHtml = applyFixes(lastAuditDoc, lastResults, newScCidValue);
